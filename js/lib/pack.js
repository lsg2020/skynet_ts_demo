...
export function decode_uint8(chunk, offset, is_le) {
    return chunk[offset];
}
export function decode_int8(chunk, offset, is_le) {
    return uncomplement(chunk[offset], 8);
}
export function decode_uint16(chunk, offset, is_le) {
    if (is_le)
        return (chunk[offset + 1] << 8) | (chunk[offset]);
    else
        return (chunk[offset] << 8) | (chunk[offset + 1]);
}
export function decode_int16(chunk, offset, is_le) {
    if (is_le)
        return uncomplement((chunk[offset + 1] << 8) | (chunk[offset]), 16);
    else
        return uncomplement((chunk[offset] << 8) | (chunk[offset + 1]), 16);
}
export function decode_uint32(chunk, offset, is_le) {
    return decode_uint(chunk, offset, 4, is_le);
}
export function decode_int32(chunk, offset, is_le) {
    return uncomplement(decode_uint(chunk, offset, 4, is_le), 32);
}
export function decode_uint(chunk, offset, n, is_le) {
    if (is_le) {
        let r = 0;
        while (n-- > 0) {
...
export function decode_biguint(chunk, offset, n, is_le) {
    if (is_le) {
...
export function decode_bigint(chunk, offset, n, is_le) {
    let bitwidth = BigInt(n * 8);
    return uncomplement_bigint(decode_biguint(chunk, offset, n, is_le), bitwidth);
}
export function decode_float(chunk, offset, is_le) {
    return readIEEE754(chunk, offset, is_le ? true : false, 23, 4);
}
export function decode_double(chunk, offset, is_le) {
    return readIEEE754(chunk, offset, is_le ? true : false, 52, 8);
}
export function decode_str(chunk, offset, n, is_le) {
    let len = decode_uint(chunk, offset, n, is_le);
    let str = utf8.read(chunk, offset + n, offset + n + len);
    return [str, offset + n + len];
}
export function sub_str(buffer, start, end) {
    return String.fromCharCode.apply(null, Array.from(buffer.subarray(start, end)));
}
export function encode_uint8(chunk, offset, value, is_le) {
    chunk[offset] = value;
}
export function encode_int8(chunk, offset, value, is_le) {
    chunk[offset] = value;
}
export function encode_uint16(chunk, offset, value, is_le) {
    if (is_le) {
...
export function encode_int16(chunk, offset, value, is_le) {
    if (is_le) {
        chunk[offset + 1] = (value >> 8) & 0xff;
        chunk[offset] = value & 0xff;
    }
...
export function encode_uint32(chunk, offset, value, is_le) {
    if (is_le)
        encode_uint(chunk, offset, value, 4, true);
    else
        encode_uint(chunk, offset, value, 4, false);
}
export function encode_int32(chunk, offset, value, is_le) {
    if (is_le)
        encode_uint(chunk, offset, value, 4, true);
    else
        encode_uint(chunk, offset, value, 4, false);
}
export function encode_safe_uint64(chunk, offset, value, is_le) {
    if (is_le)
        encode_uint(chunk, offset, value, 8, true);
    else
        encode_uint(chunk, offset, value, 8, false);
}
export function encode_safe_int64(chunk, offset, value, is_le) {
    if (is_le)
        encode_uint(chunk, offset, value, 8, true);
    else
        encode_uint(chunk, offset, value, 8, false);
}
export function encode_uint(chunk, offset, value, n, is_le) {
    if (is_le) {
        while (n-- > 0) {
...
export function encode_bigint(chunk, offset, value, n, is_le) {
    if (is_le) {
        while (n-- > 0) {
...
export function encode_float(chunk, offset, value, is_le) {
    return writeIEEE754(chunk, value, offset, is_le ? true : false, 23, 4);
}
export function encode_double(chunk, offset, value, is_le) {
    return writeIEEE754(chunk, value, offset, is_le ? true : false, 52, 8);
}
export function encode_cstring(chunk, offset, value) {
    let [sz] = utf8.write(value, chunk, offset);
    chunk[offset + sz] = 0;
    return offset + sz + 1;
}
export function encode_buf(chunk, offset, value, n, is_le) {
    if (n) {
...
function uncomplement_bigint(val, bitwidth) {
...
function readIEEE754(buffer, offset, isLittleEndian, mLen, nBytes) {
    let e;
    let m;
...
function writeIEEE754(buffer, value, offset, isLittleEndian, mLen, nBytes) {
    let e;
    let m;
    let c;
