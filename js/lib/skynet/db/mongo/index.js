...
function op_query(request_id, flags, collection, skip, ret_number, documents) {
...
function op_insert(flags, collection, documents) {
    let buf = new pack.encoder();
    buf.reserve(4);
    buf.int32(0, true);
    buf.int32(0, true);
...
function op_get_more(request_id, collection, limit, cursor_id) {
    let buf = new pack.encoder();
    buf.reserve(4);
    buf.int32(request_id, true);
    buf.int32(0, true);
...
function op_reply(msg, sz) {
    let header_len = 4 * 8;
    if (sz < header_len) {
        return [false];
    }
    let pos = 0;
    let request_id = pack.decode_uint32(msg, pos, true);
    pos += 4;
    let response_id = pack.decode_uint32(msg, pos, true);
    pos += 4;
    let opcode = pack.decode_uint32(msg, pos, true);
    pos += 4;
    let flags = pack.decode_uint32(msg, pos, true);
    pos += 4;
    if (flags & REPLY_QUERYFAILURE) {
        return [false, response_id];
    }
    let cursor_id = pack.decode_bigint(msg, pos, 8, true);
    pos += 8;
    let starting = pack.decode_uint32(msg, pos, true);
    pos += 4;
    let n = pack.decode_uint32(msg, pos, true);
    pos += 4;
    let results = [];
    while (pos + 4 < sz) {
...
function op_update(collection, flags, datas) {
    let buf = new pack.encoder();
    buf.reserve(4);
    buf.int32(0, true);
    buf.int32(0, true);
...
function op_delete(collection, single, datas) {
    let buf = new pack.encoder();
    buf.reserve(4);
    buf.int32(0, true);
    buf.int32(0, true);
...
async function dispatch_reply(sock) {
...
class MongoCollection {
    constructor(collection, db, client) {
    ...
    update(selector, update, upsert, multi) {
    ...
    async safe_update(selector, update, upsert, multi) {
...
class MongoCursor {
    constructor(collection, query, selector) {
        this._limit = 0;
        this._skip = 0;
        this._eof = false;
    ...
    async count(with_limit_and_skip) {
        let params = ["query", this._query];
...
MongoClient._auth_method = new Map([
    ["mongodb_cr", async (db, username, password) => {
    ...
    ["scram_sha1", async (db, username, password) => {
            ...
            let salt_password = (password, salt, iter) => {
