...
function buffer_find(buffer, start, end, dst) {
...
export async function recvheader(readbytes, lines, header, header_sz) {
...
export function parseheader(lines, from, header) {
    let name, value;
...
async function chunksize(readbytes, body, body_sz) {
    while (true) {
...
async function readcrln(readbytes, body) {
...
function buffer_copy(dst, offset, src) {
...
export async function recvchunkedbody(readbytes, bodylimit, header, body) {
    let size = 0;
...
export async function request(socket_interface, req) {
    let header_content = "";
    if (req.header) {
