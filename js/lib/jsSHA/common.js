import { getStrConverter, getOutputConverter } from "jsSHA/converters";
 ...
 */
export function packedLEConcat(a, b) {
    let i, arrOffset;
    const aByteLen = a["binLen"] >>> 3, bByteLen = b["binLen"] >>> 3, leftShiftAmount = aByteLen << 3, rightShiftAmount = (4 - aByteLen) << 3;
 ...
 */
export function parseInputOption(key, value, bigEndianMod, fallback) {
...
export class jsSHABase {
    ...
    getHash(format, options) {
        let i, finalizedState, outputBinLen = this.outputBinLen;
    ...
    _getHMAC() {
        ...
        const firstHash = this.finalizeFunc(this.remainder.slice(), this.remainderLen, this.processedLen, this.stateCloneFunc(this.intermediateState), this.outputBinLen);
