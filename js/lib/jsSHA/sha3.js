import { jsSHABase, packedLEConcat, sha_variant_error, mac_rounds_error, TWO_PWR_32, parseInputOption } from "jsSHA/common";
import { getStrConverter } from "jsSHA/converters";
import { Int_64, rotl_64, xor_64_2, xor_64_5 } from "jsSHA/primitives_64";
const rc_sha3 = [
...
function left_encode(x) {
    ...
    const retVal = [0, 0], x_64 = [x & 0xffffffff, (x / TWO_PWR_32) & 0x1fffff];
...
function byte_pad(packed, outputByteLen) {
    let encodedLen = left_encode(outputByteLen), i;
...
export default class jsSHA extends jsSHABase {
    ...
    constructor(variant, inputFormat, options) {
        ...
        this.finalizeFunc = function (remainder, remainderBinLen, processedBinLen, state, outputBinLen) {
            return finalizeSHA3(remainder, remainderBinLen, processedBinLen, state, variantBlockSize, delimiter, outputBinLen);
        };
    ...
    _initializeCSHAKE(options, funcNameOverride) {
        ...
        const packedParams = packedLEConcat(encode_string(resolvedOptions["funcName"]), encode_string(resolvedOptions["customization"]));
        ...
        if (resolvedOptions["customization"]["binLen"] !== 0 || resolvedOptions["funcName"]["binLen"] !== 0) {
            ...
            for (let i = 0; i < byte_pad_out.length; i += this.variantBlockSize >>> 5) {
                this.intermediateState = this.roundFunc(byte_pad_out.slice(i, i + (this.variantBlockSize >>> 5)), this.intermediateState);
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    _getKMAC(options) {
        const concatedRemainder = packedLEConcat({ value: this.remainder.slice(), binLen: this.remainderLen }, right_encode(options["outputLen"]));
        return this.finalizeFunc(concatedRemainder["value"], concatedRemainder["binLen"], this.processedLen, this.stateCloneFunc(this.intermediateState), options["outputLen"]);
    }
}
