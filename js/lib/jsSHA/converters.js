...
function str2packed(str, utfType, existingPacked, existingPackedLen, bigEndianMod) {
    let codePnt, codePntArr, byteCnt = 0, i, j, intOffset, byteOffset, shiftModifier, transposeBytes;
...
function b642packed(str, existingPacked, existingPackedLen, bigEndianMod) {
    let byteCnt = 0, index, i, j, tmpInt, strPart, intOffset, byteOffset;
    existingPackedLen = existingPackedLen || 0;
 ...
 */
export function getStrConverter(format, utfType, bigEndianMod
 ...
 */
export function packed2hex(packed, outputLength, bigEndianMod, formatOpts) {
 ...
 */
export function packed2b64(packed, outputLength, bigEndianMod, formatOpts) {
    let str = "", i, j, triplet, int1, int2;
    const length = outputLength / 8, shiftModifier = bigEndianMod === -1 ? 3 : 0;
 ...
 */
export function packed2bytes(packed, outputLength, bigEndianMod) {
    let str = "", i, srcByte;
    const length = outputLength / 8, shiftModifier = bigEndianMod === -1 ? 3 : 0;
    for (i = 0; i < length; i += 1) {
 ...
 */
export function packed2arraybuffer(packed, outputLength, bigEndianMod) {
    let i;
 ...
 */
export function packed2uint8array(packed, outputLength, bigEndianMod) {
    let i;
 ...
 */
export function getOutputConverter(format, outputBinLen, bigEndianMod, outputOptions
/* eslint-disable-next-line @typescript-eslint/no-explicit-any */
) {
    switch (format) {
        case "HEX":
