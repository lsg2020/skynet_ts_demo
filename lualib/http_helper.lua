...
local function response(id, code, resp)
    ...
    if not ok then
        print("swt http response error fd = %d, %s", id, err)
    end
    socket.close(id)
end

local function upgrade(handle_agent, request)
    local ok, err = websocket.accept(request.id, handle_agent, "ws", request.addr, {upgrade = {header = request.headers, url = request.url, method = "GET"}})
...
local function dispatch(router, id, addr)
    ...
    if not code then
        if url ~= sockethelper.socket_error then
            print("swt http read error: %s", url)
        end
        socket.close(id)
        return
    end
