...
function fakeParseJSON(str: string) {
    ...
    function parseObject() {
      ...
      function parseKeyword(name: any, value: any) {
      ...
      function isHexadecimal(char: any) {
        return (
          (char >= "0" && char <= "9") ||
      ...
      // error handling
      function expectNotEndOfInput(expected: any) {
        if (i === str.length) {
    ...
    { "foo": "bar" }
      ...
      function expectCharacter(expected: any) {
        if (str[i] !== expected) {
          printCodeSnippet(`Expecting a \`${expected}\` here`);
          throw new Error("JSON_ERROR_0004 Unexpected token");
        }
      }
      function expectDigit(numSoFar: any) {
        if (!(str[i] >= "0" && str[i] <= "9")) {
    ...
    ${" ".repeat(numSoFar.length)}^`);
      ...
      function expectEscapeCharacter(strSoFar: any) {
    ...
    List of escape characters are: \\", \\\\, \\/, \\b, \\f, \\n, \\r, \\t, \\u`);
      ...
      function expectEscapeUnicode(strSoFar: any) {
    ...
    ${" ".repeat(strSoFar.length + 1)}^^^^^^`);
      ...
      function printCodeSnippet(message: any) {
...
skynet.start(() => {
    ...
    for (let i = 1; i <= amount; i++) {
        let r = fakeParseJSON(`{"a": 1234, "b": [1, 2, 3, 4, ${i}], "c": {"a": 1234, "b": [1, 2, 3, 4], "c": {}}}`)
        result.push(r)
    }
    let ts_end = Date.now();

    console.log(`\n\nJS json decode result \tamount:${result.length}\t\tms:${ts_end - ts_begin}\n\n`);
